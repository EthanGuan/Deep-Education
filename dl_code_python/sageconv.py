...
class GraphSage(nn.Module):
    ...
    def forward(self, graph, feat, subgraph, weight=None):
        ...
        if self._in_feats > self._out_feats:
            ...
            rst = sparse.run_graphsage(graph, feat1, self._norm, num_vcount, dim, self.sample_num, subgraph)
            #print(rst)
        else:
            dim = feat.size(1)
            # aggregate first then mult W
            rst = sparse.run_graphsage(graph, feat, self._norm, num_vcount, dim, self.sample_num, subgraph)
            if weight is not None:
...
class GraphSageNet(nn.Module):
    def __init__(self, graph, in_feats, hidden_size, num_classes):
        ...
        self.graph = graph
        self.subgraph = kernel.init_like(graph)
        self.conv1 = GraphSage(in_feats, hidden_size, sample_num=10)
        self.conv2 = GraphSage(hidden_size, num_classes, sample_num=25)
    def forward(self, inputs):
        h = self.conv1(self.graph, inputs, self.subgraph)
        h = th.relu(h)
        h = self.conv2(self.graph, h, self.subgraph)
        return h
